...
const Settings: React.FC<SettingsProps> = ({ onClose, syncStatus, syncProgress, onManualSync }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-black/60 backdrop-blur-md transition-opacity duration-300">
      <div className="bg-background rounded-2xl shadow-2xl w-full max-w-3xl max-h-[85vh] overflow-hidden flex flex-col ring-1 ring-white/10">
        {/* Header */}
        ...
        <div className="px-3 sm:px-6 pb-2 pt-2 bg-surface">
          <div className="flex p-1 bg-surface2 rounded-xl overflow-x-auto no-scrollbar">
            {tabs.map((tab) => (
              <button
                key={tab.id}
        ...
        {/* Content Area */}
        <div className="flex-1 overflow-y-auto px-4 py-4 sm:px-6 sm:py-6 min-h-[300px] sm:min-h-[400px] bg-background">
          {/* 词汇等级 */}
          {activeTab === 'vocabulary' && (
            <div className="space-y-6 sm:space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-300">
              <section>
                <h3 className="text-base sm:text-lg font-semibold text-foreground mb-1 sm:mb-2">词汇量等级</h3>
                      ...
                      >
                        <div className="flex justify-between items-start">
                          <div>
                            ...
                            <div className="mt-2 sm:mt-3 inline-flex items-center px-1.5 py-0.5 sm:px-2 sm:py-1 rounded bg-surface border border-border text-[10px] sm:text-xs font-medium text-muted">
              ...
              <section>
                <h3 className="text-base sm:text-lg font-semibold text-foreground mb-1 sm:mb-2">词典范围</h3>
                <p className="text-xs sm:text-sm text-muted mb-3 sm:mb-4">
                  ...
                  {['small', 'large'].map((size) => (
                    ...
                    >
                      <div className="text-left">
                        <h4 className="font-bold text-sm sm:text-base text-foreground capitalize">{size} Dictionary</h4>
            ...
            <div className="space-y-4 sm:space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
              ...
              {newWords.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-12 sm:py-16 text-center">
                  <div className="w-16 h-16 sm:w-20 sm:h-20 bg-surface2 rounded-full flex items-center justify-center mb-3 sm:mb-4">
                    <BookOpen className="text-muted w-8 h-8 sm:w-10 sm:h-10" />
                  </div>
                  <p className="text-base sm:text-lg font-medium text-foreground">空空如也</p>
                  <p className="text-xs sm:text-sm text-muted mt-1 sm:mt-2 max-w-xs mx-auto">
                ...
                <div className="space-y-4 sm:space-y-6">
                  {(Object.entries(newWordsByBook) as [string, NewWord[]][]).map(([bookTitle, words]) => (
                    <div key={bookTitle} className="group bg-surface2/50 rounded-xl border border-border overflow-hidden hover:shadow-lg transition-shadow duration-300">
                      <div className="px-3 py-2.5 sm:px-4 sm:py-3 bg-surface2 border-b border-border flex justify-between items-center">
                        <div className="flex items-center gap-2">
                          <Book size={14} className="text-muted sm:w-4 sm:h-4" />
                          <h4 className="font-semibold text-sm sm:text-base text-foreground truncate max-w-[150px] sm:max-w-sm">
                            {bookTitle}
                          </h4>
                          <span className="px-1.5 py-0.5 bg-background text-muted text-[10px] sm:text-xs rounded-full">
                            {words.length}
                          </span>
                        </div>
                        <button
                        ...
                        <div className="flex flex-wrap gap-1.5 sm:gap-2">
                          {words.slice(0, 15).map((word, idx) => (
                            ...
                            >
                              ...
                              {word.translation && (
                                <span className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-2 py-1 bg-slate-900 text-white text-[10px] sm:text-xs rounded opacity-0 group-hover/word:opacity-100 transition-opacity pointer-events-none whitespace-nowrap z-10 hidden sm:block">
                          ...
                          {words.length > 15 && (
                            <div className="text-xs sm:text-sm text-muted px-2 py-1 flex items-center">
                              +{words.length - 15} ...
                            </div>
                          )}
                        </div>
          ...
          {activeTab === 'webdav' && (
            <div className="space-y-4 sm:space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
              ...
              <div className="space-y-4 sm:space-y-5">
                <div className="space-y-3 sm:space-y-4">
                  <div className="group">
                    ...
                    <div className="relative">
                      <Globe className="absolute left-3 top-1/2 -translate-y-1/2 text-muted w-4 h-4 sm:w-[18px] sm:h-[18px]" />
                      <input
                        type="text"
                ...
                <div className="flex items-center justify-between p-3 sm:p-4 bg-surface2 rounded-xl border border-border">
                  <div className="flex flex-col">
                    <span className="text-xs sm:text-sm font-medium text-foreground">自动同步</span>
                    <span className="text-[10px] sm:text-xs text-muted">数据变更时自动上传</span>
                  </div>
                ...
                <div className="flex gap-3">
                  ...
                  <button
                    ...
                    className="flex-1 px-3 py-2 sm:px-4 sm:py-2.5 bg-surface text-foreground border border-border rounded-lg hover:bg-surface2 disabled:opacity-50 disabled:cursor-not-allowed transition-all active:scale-[0.98] flex items-center justify-center gap-1.5 sm:gap-2 text-xs sm:text-sm font-medium"
                  >
                    {isTesting ? (
                ...
                {(testResult || syncStatus) && (
                  <div className="animate-in fade-in slide-in-from-top-1 duration-200">
                    ...
                    {syncStatus && (
                      <div className="p-3 sm:p-4 bg-surface2 rounded-xl border border-border flex items-center justify-between">
                        <div className="flex items-center gap-2 sm:gap-3">
                          ...
                          <div className="flex flex-col">
                            ...
                            {syncProgress?.message && (
                              <span className="text-[10px] sm:text-xs text-muted">{syncProgress.message}</span>
                            )}
                          </div>
                        </div>
                        <button
